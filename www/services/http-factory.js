...
ifameApp.factory('httpFactory', [
    ...
    function($http, appConstants, appGlobals, $q) {
        ...
        //
        factory.apiPost_1 = function(action, data) {
            var req = {
                method: 'POST',
                url: appConstants.BASEURL_1 + action,
                headers: {
                    'Authorization': appGlobals.usrAuthen
                },
                data: data
        ...
        //
        factory.apiPostTimeout = function(action, data, sec) {
            var self = this;
            var timeout = $q.defer();
            var result = $q.defer();
            var timeOut = false;
            var secTimeout = ( (angular.isUndefined(sec) || sec === null) ? 5 : sec);
            //var secDelay = 3;
            //
            var req = {
                method: 'POST',
